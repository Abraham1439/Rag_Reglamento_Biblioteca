...
5. **Agente RAG + Filtro de Confianza**: Lógica conversacional (`BibliotecaDuocAgent`) que evalúa la puntuación de confianza ($1 / (1 + \text{distancia L2})$). Si el score es inferior al umbral (`CONFIDENCE_THRESHOLD = 0.45`), el agente **deriva la consulta a un encargado humano** en vez de responder.
